...
public class TrackerService extends Service implements SensorEventListener, AlarmManager.OnAlarmListener {
    ...
    private final double[] mAccelLast = {0.0, 0.0, 0.0}; // Keeps previous sensor value
    private final double mSleepMotionMean = -1;
    ...
    private final Runnable mTrackingRunnable = new Runnable() {
        ...
        public void run() {
            ...
            mTrackingHandler.postDelayed(this, (long) pollRate * 60 * 1000);
        }
    };
            ...
            case "alarm":
                ...
                if (mSmartAlarm != null) {
                    ...
                    if (now.after(mSmartAlarm.getTime())) {
                        activateAlarm();
                    } else if (mSmartAlarm.getTime().getTime() - now.getTime() <= (long) window * 60 * 1000) {
    ...
    private void activateAlarm() {
        if (mPreferences.getBoolean("smartalarm_use", true)) {
            ...
            Date now = Calendar.getInstance().getTime();
            configureAlarm(now.before(mSmartAlarm.getTime()));
    ...
    public void play() {
        ...
        if (!mPreferences.getBoolean("datapoint_forceaccurate", true)) {
            ...
            AlarmManager alarmManager = (AlarmManager) this.getSystemService(Context.ALARM_SERVICE);
            int pollRate = mPreferences.getInt("datapoint_rate", 1);
            alarmManager.setRepeating(AlarmManager.RTC_WAKEUP, System.currentTimeMillis(), pollRate * 60000L, pingIntent);
        }
